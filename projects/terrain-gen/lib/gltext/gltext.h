...
//    !!! NOTICE !!!
//
//    this source file has been modified from 
//    the original for opengl 2.0 ES support
...
GLT_API GLTtext* gltCreateText(void)
{
	...
	/*MODIFIED*/// glGenVertexArrays(1, &text->_vao);
	glGenBuffers(1, &text->_vbo);

	/*MODIFIED*///_GLT_ASSERT(text->_vao);
	_GLT_ASSERT(text->_vbo);

	/*MODIFIED*/if (/*!text->_vao ||*/ !text->_vbo)
	{
		gltDeleteText(text);
		return GLT_NULL;
	}

	/*MODIFIED*/// glBindVertexArray(text->_vao);
...
GLT_API void gltDeleteText(GLTtext *text)
{
	...
	if (text->_vao)
	{
		/*MODIFIED*///glDeleteVertexArrays(1, &text->_vao);
...
/*MODIFIED*/"#version 300 es\n"
/*MODIFIED*/"precision mediump float;\n"
/*MODIFIED*///"#version 330 core\n"
"\n"
"in vec2 position;\n"
"in vec2 texCoord;\n"
"\n"
...
"\n"
/*MODIFIED*///"uniform vec4 color = vec4(1.0, 1.0, 1.0, 1.0);\n"
/*MODIFIED*/"uniform vec4 color;\n"
"\n"
"in vec2 fTexCoord;\n"
"\n"
"void main()\n"
...
GLT_API GLboolean _gltCreateText2DShader(void)
{
    ...
    /*MODIFIED*/// Without the following, make sure fragColor is the first out variable
	/*MODIFIED*///glBindFragDataLocation(_gltText2DShader, 0, "fragColor");
